...
export class User1638806858251 implements MigrationInterface {
  name = "User1638806858251";
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    try {
      ...
      const hash = crypto
        .pbkdf2Sync("heslo123", salt, 1000, 64, `sha512`)
        .toString(`hex`);

      await queryRunner.query(
        `INSERT INTO "user" ("id", "userName", "firstName", "lastName", "age", "password", "salt", "customerId", "roleId") VALUES (1, "janko", "hrasko", "zeleny", 12, "${hash}", "${salt}", NULL, "1")`
      );
    } catch (error) {
      console.log(error);
    }
